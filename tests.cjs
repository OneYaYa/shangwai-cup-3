...
test('published public data validates and produces the expected score',()=>{const d=S.validate(JSON.parse(fs.readFileSync(__dirname+'/results.json','utf8'))),w=S.workbook(d.records,d.policy);assert(Object.keys(d.records).length>=1);assert.equal(w.scores.lan.settlement,2809.2);assert(w.teams.every(t=>t.entered===0));});

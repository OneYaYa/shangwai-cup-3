...
test('first clear never enters settlement even with legacy policy',()=>{const records={teddy:r('teddy',{base:1000,pain:true})};for(const firstClear of ['team','before','after']){const w=S.workbook(records,{...C.defaultPolicy,firstClear});assert.equal(w.scores.teddy.settlement,1140);assert.equal(w.scores.teddy.teamLevel,0);assert.equal(w.teams.find(t=>t.id==='teddy').teamBonus,100);assert.equal(w.teams.find(t=>t.id==='teddy').total,1240);}});
test('D violations always deducted after multiplication',()=>{const rec=r('narrative',{base:2000,violations:1,pain:true});for(const dPenalty of ['before','after']){const s=S.calculate(p('narrative'),rec,{policy:{dPenalty}});assert.equal(s.settlement,1420);assert.equal(s.teamLevel,0);assert.equal(s.total,1420);}});
...
test('published public data validates and produces the expected score',()=>{const d=S.validate(JSON.parse(fs.readFileSync(__dirname+'/results.json','utf8'))),w=S.workbook(d.records,d.policy),t=w.teams.find(t=>t.id==='teddy');assert(Object.keys(d.records).length>=1);assert.equal(w.scores.lan.settlement,2809.2);assert.equal(w.scores.mumu.settlement,3294);assert.equal(w.scores.mumu.teamLevel,50);assert.equal(t.teamBonus,150);assert.equal(t.total,3444);});
test('split ledger reconciles team total with unique rewards and penalties',()=>{const records={duck:r('duck',{base:1000,parts:4,wheel:true,belly:true,hunt_arm:true,pain:true}),bridge:r('bridge',{base:500,hunt_arm:true,pain:true}),narrative:r('narrative',{base:2000,violations:1,chaos:true})};const w=S.workbook(records),t=w.teams.find(t=>t.id==='duck');assert.equal(w.scores.duck.settlement,1570);assert.equal(w.scores.duck.teamLevel,60);assert.equal(w.scores.bridge.settlement,700);assert.equal(w.scores.bridge.teamLevel,0);assert.equal(w.scores.narrative.settlement,1475);assert.equal(w.scores.narrative.teamLevel,0);assert.equal(t.settlementTotal,3745);assert.equal(t.levelTotal,60);assert.equal(t.teamBonus,260);assert.equal(t.total,4005);});
test('old saved policies migrate to confirmed settings without record loss',()=>{const rec=r('duck',{base:1000,wheel:true,belly:true,pain:true});const d=S.validate({schemaVersion:1,records:{duck:rec},policy:{factions:'best',relics:'combined',dPenalty:'before',firstClear:'before'}});assert.deepEqual(d.policy,C.defaultPolicy);assert.equal(d.records.duck.base,1000);assert.equal(S.workbook(d.records,d.policy).scores.duck.settlement,1600);});
test('zero and unentered records remain distinct and no bonus is duplicated',()=>{const w=S.workbook({duck:r('duck',{base:0,pain:true}),bridge:r('bridge',{pain:true,hunt_arm:true})});const t=w.teams.find(t=>t.id==='duck');assert.equal(t.entered,1);assert.equal(t.settlementTotal,200);assert.equal(t.teamBonus,100);assert.equal(t.total,300);});
...
test('boss rewards score independently per player while first clear remains team-only',()=>{
 ...
 assert.equal(w.scores.bridge.teamLevel,0);assert.equal(w.scores.bridge.firstBonus,0);assert.equal(w.teams.find(t=>t.id==='duck').teamBonus,100);
 assert.equal(w.scores.duck.settlement,1300);assert.equal(w.scores.bridge.settlement,1500);assert(w.scores.duck.lines.some(l=>l.kind==='first'));assert(!w.scores.duck.warnings.some(x=>x.includes('最高完整奖励')));
});
test('disease packages score independently for each player',()=>{
 ...
 assert.equal(w.scores.duck.settlement,375);assert.equal(w.scores.bridge.settlement,350);assert.equal(w.scores.duck.teamLevel,0);assert.equal(w.scores.bridge.teamLevel,0);assert.equal(w.teams.find(t=>t.id==='duck').teamBonus,0);
});
test('boss records remain isolated by team for public attribution only',()=>{
 ...
 assert.equal(w.scores.duck.settlement,450);assert.equal(w.scores.bridge.settlement,550);assert.equal(w.scores.mumu.settlement,200);assert.equal(w.scores.duck.firstBonus,200);assert.equal(w.bossOwners['duck:chaos'],'bridge');assert.equal(w.bossOwners['teddy:pain'],'mumu');
});
test('unentered boss results cannot claim public record or first and corrections reassign',()=>{
 const records={duck:r('duck',{pain:true,pain_hunt:true}),bridge:r('bridge',{base:0,pain:true})};
 let w=S.workbook(records);assert.equal(w.bossOwners['duck:pain'],'bridge');assert.equal(w.scores.bridge.firstBonus,100);
 records.duck.base=0;w=S.workbook(records);assert.equal(w.bossOwners['duck:pain'],'duck');assert.equal(w.scores.duck.settlement,400);assert.equal(w.scores.duck.firstBonus,100);
 records.duck.base=null;w=S.workbook(records);assert.equal(w.bossOwners['duck:pain'],'bridge');assert.equal(w.scores.bridge.firstBonus,100);
});
test('boss and box bonuses multiply personally while team levels remain raw',()=>{
 ...
 assert.equal(s.extra,520);assert.equal(s.multiplier,1.15);assert.equal(s.settlement,1613.5);assert.equal(s.teamLevel,70);assert.equal(s.total,1683.5);
});
test('box reward can be earned once by each player without entering team bonus',()=>{
 const w=S.workbook({duck:r('duck',{base:0,box:true}),bridge:r('bridge',{base:0,box:true})}),t=w.teams.find(t=>t.id==='duck');
 assert.equal(w.scores.duck.settlement,70);assert.equal(w.scores.bridge.settlement,70);assert.equal(w.scores.duck.teamLevel,0);assert.equal(t.teamBonus,0);assert.equal(t.total,140);
});

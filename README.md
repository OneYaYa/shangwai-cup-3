...
| 违规扣分 | 完成倍率计算后扣除 |
| 首次结局奖励 | 痛苦将息、混沌源阶理论队内各 +100，仅加入团队总加分，不乘个人倍率 |

竞技成绩按两类账目记录与展示：

- 选手结算分 =（游戏结算分 + 结算加分 − 7.5 × 零件数）× 总倍率 − 违规扣分 + 有理由的裁判调整。
- 本场团队关卡分 = 关卡规则分 × 该选手总倍率。保留原规则倍率关系。
- 团队总加分 = 各场团队关卡分之和 + 首次结局奖励。
- 团队总分 = 选手结算分合计 + 团队总加分。

结算加分包括机械师、抓位、文明开化补偿、箱中猎影和限制奖励。关卡规则分包括道中追猎、结局、特殊关卡、同域共存和沙盘/三供奖励。队内唯一奖励按已录入选手的赛程顺序去重。每场结算分和关卡贡献分别保留两位小数后相加，确保展示账目与总分一致。未填写结算分的记录不参与团队统计。

`scoring.js` 的 `settlement` 为选手结算分，`teamLevel` 为本场团队关卡分；`total` 保留两者之和供内部核算。团队对象的 `settlementTotal`、`levelTotal`、`firstBonus`、`teamBonus` 分别表示个人结算合计、关卡贡献合计、首通奖励和团队总加分。展示不再将关卡贡献混入个人结算分。
